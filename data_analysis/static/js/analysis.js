(function () {
  const saved = localStorage.getItem('theme') || 'amoled';
  document.documentElement.setAttribute('data-theme', saved);
  var _themeOrder = ['amoled', 'light', 'dark'];
  var _themeLabels = { amoled: '🖤 AMOLED', light: '☀️ Light', dark: '🌙 Dark' };
  function setThemeButtonLabel() {
    const btn = document.getElementById('themeToggle');
    if (!btn) return;
    const cur = document.documentElement.getAttribute('data-theme') || 'amoled';
    btn.textContent = _themeLabels[cur] || '🖤 AMOLED';
  }
  window.toggleTheme = function () {
    const cur = document.documentElement.getAttribute('data-theme') || 'amoled';
    const idx = _themeOrder.indexOf(cur);
    const next = _themeOrder[(idx + 1) % _themeOrder.length];
  ...
  // Helper to detect dark mode (dark or amoled)
  window.isDarkMode = function () {
    const theme = document.documentElement.getAttribute('data-theme');
    if (theme) return theme === 'dark' || theme === 'amoled';
